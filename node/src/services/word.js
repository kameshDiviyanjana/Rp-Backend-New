...
export const getAllwordsService = async (query) => {
  let { page, limit, searchTerm, userme, pacticesword } = query;
  page = parseInt(page, 10) || 1;
  limit = parseInt(limit, 10) || 10;
  searchTerm = searchTerm || "";

  try {
    const words = await getAllwordsesRepo({
      page,
      limit,
      searchTerm,
      userme,
...
export const removewordService = async (id) => {
  try {
    const word = await removewordseById({ ...id });
...
export const findwordbyid = async (id) => {
  try {
    const word = await findword({ _id:id });
    if (!word) throw new createError(404, "word not found");
    return word;
  } catch (error) {
    // throw error;
